...
void main() {
  testWidgets('messages an on-WhatsWave contact and pops with the thread id',
    ...
    expect(
        find.byKey(const Key('new_chat_call_audio_ava-patel')), findsOneWidget);
    expect(
        find.byKey(const Key('new_chat_call_video_ava-patel')), findsOneWidget);
  ...
  testWidgets(
      'picking a group icon on the name step uploads it right after '
      'creation', (tester) async {
    ImagePickerPlatform.instance = FakeImagePickerPlatform(
      singleImagePath: '/fake/group-icon.jpg',
    );
    ...
    expect(find.byKey(const Key('new_group_details_screen')), findsOneWidget);
    // Before picking, the preview is the initials badge (no local file
    // preview yet).
    ...
    // exists yet to upload it against.
    ...
    final thread = chatsController.threadById(capturedThreadId!);
    expect(thread?.name, 'Launch Crew');
    expect(thread?.avatarUrl, '/fake/group-icon.jpg');
  });
