...
6. [x] Add media upload/download -- `FirebaseStatusMediaStore` (`lib/features/updates/data/status_media_store.dart`) uploads status photo/video segments to Cloud Storage via `FirebaseMediaUploader`, same as chat attachments, falling back to a device-local copy only if the upload itself fails
...
- **Updates**: `FirestoreUpdatesRepository` (`lib/features/updates/data/firestore_updates_repository.dart`) -- one `statusStories/{uid}` document per user (leverages `StatusStory.toJson()`/`fromJson()`, which already serialized the entire nested segment/overlay/transform model). Media uploads to Cloud Storage (see the checklist above); `status_media_prefetch.dart` warms the image/video cache right after a successful post so the poster's own first view doesn't cold-fetch what it just uploaded.
...
- **Calls (Slice B2 -- Firestore signaling)**: `CallSignalingService`/`FirestoreCallSignalingService` (`lib/features/calls/data/`) -- a new `calls/{callId}` Firestore collection lets one device notify another that a call is happening (ringing/accepted/declined/ended), independent of media. A debug-only `CallSignalingTestScreen` (Settings -> Backend and sync -> Debug tools -> "Test call signaling") exercises it directly. **Verified with a real two-device test** (iOS Simulator + a physical Android phone, two separate Firebase accounts): placing a call from one device surfaced it live on the other within ~1-2 seconds, and accept/status changes synced back correctly. Needed a composite Firestore index (documented in `firestore.indexes.json`) and an Android build fix (`google-services` plugin bumped to 4.5.0 -- the version FlutterFire CLI had pinned was too old for the already-present Crashlytics Gradle plugin).
- **Calls (Slice B3 -- wired into CallsController)**: `CallsController` (`lib/features/calls/application/calls_controller.dart`) now runs a real call end-to-end whenever the callee's `CallContact.uid` is known -- real Firestore signaling (B2) plus a real LiveKit room joined with a token from the B1 server, replacing the Timer-based simulation for that path only (contacts without a `uid` still get the original simulated flow, unchanged). `call_experience_screen.dart` renders real local/remote video via `VideoTrackRenderer` for real calls. To actually reach this from the UI, Communities' contact list (`lib/features/communities/presentation/communities_screen.dart`) grew real Voice/Video call buttons on contacts already confirmed "on WhatsWave" -- this required threading the previously-discarded matched uid through `CommunityContact.matchedUid`. Known, documented gaps: incoming calls show a placeholder caller name (no contact-name resolution for arbitrary callers yet), and a second simultaneous incoming call is silently ignored rather than queued.

  **Fully verified with a real two-device call** (iOS Simulator + a physical Android phone), including real audio and video both directions. Three real bugs surfaced only by that live test, all fixed in `CallsController`/`WhatsWaveApp`:
  1. Real incoming calls never auto-opened the call screen -- every other way of starting a call pushes explicitly from a button handler's `BuildContext`, but an incoming call has none. Fixed with a root-level listener in `WhatsWaveApp` (via a `navigatorKey`, since there's no local context to push from), scoped tightly to real incoming calls so it never double-pushes over an existing explicit push.
  2. No audio was audible on either side despite tracks publishing correctly (confirmed via the LiveKit Cloud dashboard's session view). Root cause, found with a live `adb logcat` capture during an actual call: Android's native audio routing picked the earpiece over the speaker despite a non-forced speaker preference, on a real device with no headset connected. Fixed by passing `force: true` specifically when preferring speaker.
  3. Even after that fix, one test still had zero audio -- caused by the iOS Simulator's own **I/O -> Audio Input** menu being set to None/Mute, so it never captured real microphone data (and never throws for this, so `setMicrophoneEnabled` reports success while capturing silence). Not a code bug -- a one-time per-machine Simulator setting anyone testing this needs to check.

Still fully local/simulated: **Settings/preferences** (no reason to move these to a backend yet). **Media/Storage** and **sending pushes** (as opposed to registering for them) are both blocked on the Blaze billing upgrade.
